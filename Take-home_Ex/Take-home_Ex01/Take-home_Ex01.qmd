...
demo_p1 <- ggplot(demo_data_new, aes(x = householdSize)) +
  ...
  labs(title = "Household Size",
       x = NULL,
       y = NULL) +
  ylim(0,420) + #< Set limit to ensure that geom_text annotation is not cut off
...
demo_patch <- ((demo_p3 + house_patch) / (demo_p5 + demo_p4)) +
              ...
              plot_annotation(title = "Summary Statistics of Demographic Data")

demo_patch & theme(axis.text.y = element_blank(), #< Remove y axis since annotations already show count
                   panel.grid.major = element_blank(), #< Remove lines in panel background for cleaner design
                   plot.background = element_rect(fill="#F8F3E6",colour="#F8F3E6")) #< Same colour as webpage
```
...
monthly_mean <- financial_pivot %>%
                  group_by(transDate) %>%
                  summarise(num_people = n_distinct(participantId),
                  ...
                  mutate(transDate = ymd(paste(transDate, "01", sep = "-"))) %>% 
                  ungroup()

# Extracting only expenditure categories
monthly_exp <- monthly_mean %>%
                  select(-num_people, -Wage, -RentAdj) %>%
                  pivot_longer(cols = !transDate, 
                            names_to = "category",
                            values_to = "amount") %>%
...
# Line plot to show 
ggplot(monthly_exp, 
       aes(x = transDate, y= amount, color = category)) + #< Ensure that each category is color coded
  geom_line(linewidth = 1) + #< Make line thicker
  geom_point(aes(x = transDate, y= amount, color = category),
             size = 1.5) +
  scale_color_manual(values = c("Shelter" = "salmon", 
                                "Education" = "#1696d2", 
                                "Food" = "#fdbf11", 
                                "Recreation" = "grey10")) +
  scale_x_date(date_labels = "%Y-%m", date_breaks = "1 month") +
  scale_y_continuous(labels = scales::dollar) + #< Ensuring that units were added to y-axis 
  guides(color = guide_legend(reverse = TRUE)) + 
  labs(title = "Sudden Dip in Expenditure on Shelter and Recreation from Mar-Apr 2022",
       x = NULL,
...
-   There were transactions from all 1011 participants in Mar-2023, but only 880 in Apr-2024. This suggests that the transactions from those 31 participants could have been skewing the data. financial data from 2022-03 will thus be excluded from further analysis.
...
exp_range <- financial_data_new %>%
                filter(category %in% c("Education", "Food", "Recreation", "Shelter")) %>%
                
              ggplot(aes(x = category, y = amount)) +
              stat_boxplot(geom = "errorbar", width = 0.2) +
              geom_boxplot(fill = "salmon") +
              scale_y_continuous(labels = scales::dollar) +
              labs(title = "Expenditure Range by Category",
...
inc_range <- financial_data_new %>%
              ...
              geom_boxplot() +
              scale_y_continuous(labels = scales::dollar) +
              labs(title = "Income Range by Category",
...
## 2.3: Combining Datasets and Computing Annual Income per Capita
...
h <- ggplot(df_combined, aes(x = incomePerCapita)) +
      ...
      scale_x_continuous(breaks = scales::pretty_breaks(n = 5),
                        limits = c(500,12000), #< Set same limits for both plots to ensure justification
                        labels = label_dollar()) + #< Label with $ unit
      geom_vline(aes(xintercept = median_incomePC),
                    colour="grey20", linewidth = 0.7, linetype = "dashed") +
      annotate(geom = "text", x = 3800, y = 290,
...
ggplot(df_combined, aes(x = joviality)) + 
      ...
      stat_function(fun = dnorm,
                ...
                col = "salmon",
                size = 1) +
      geom_vline(aes(xintercept = mean_jov),
                colour="#4d5887", linewidth = 0.8, linetype = "dashed") +
      annotate(geom = "text", x = mean_jov + 0.1, y = 1.5,
                  label = paste0("Mean Joviality: ", round((mean_jov),2)),
                  color = "#4d5887") +
      geom_vline(aes(xintercept = med_jov),
                colour="grey20", linewidth = 0.8, linetype = "dashed") +
      annotate(geom = "text", x = med_jov - 0.1, y = 1.5,
                  label = paste0("Median Joviality: ", round((med_jov),2)),
                  color = "grey20") +
      scale_x_continuous(breaks = scales::pretty_breaks(n = 10),
                         limits = c(0,1)) +
      labs(title = "Uneven Distribution of Joviality Score across Participants",
           subtitle = "Median joviality score is close to Mean score, but histogram reveals multiple peaks and troughs") +
      theme(axis.text.y = element_blank(),
...
ggplot(df_combined, 
      ...
      facet_wrap(~ incomeGroup, nrow = 1) + #< nrow used to fit all plots into a line
...
```{r}
#| fig-width: 8
ggplot(df_combined, aes(x = joviality, fill = ageGroup, color = ageGroup)) +
