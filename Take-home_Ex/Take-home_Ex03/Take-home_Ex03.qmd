...
Distribution of company revenue as well as quantile values show a **highly right-skewed distribution**, which could be an indication of company size. To use this variable for further classification of companies, revenue is binned by percentile and assigned a label:  
...
percentiles <- quantile(company_nodes$revenue_omu, 
                        probs = c(0, 0.2, 0.4, 0.6, 0.8, 1),
                        na.rm = TRUE)

# Create a new column and assign labels based on percentiles
company_nodes$revenue_group <- cut(company_nodes$revenue_omu, 
                                   breaks = percentiles, 
                                   labels = c(5, 4, 3, 2, 1), 
                                   include.lowest = TRUE)
```
...
mc3_links_new <- mc3_links %>%
  inner_join(links_count, 
             by = c("source" = "source", "type" = "type")) %>%
  select(source, target, type, count)
```

```{r}
# Select only people connected to >5 companies
most_connected <- links_targetcount %>%
  filter(company_count >7)

# Create nodes dataframe from filtered links
mc_source <- most_connected %>%
  distinct(source) %>%
  rename("id" = "source")
mc_target <- most_connected %>%
  distinct(target) %>%
  rename("id" = "target")

most_connected_nodes <- bind_rows(mc_source, mc_target)
```

```{r}
most_connected_graph <- tbl_graph(nodes = most_connected_nodes,
                          edges = most_connected, 
                          directed = TRUE)
most_connected_graph
```
```{r}


ggraph(most_connected_graph, 
            layout = "nicely") + 
