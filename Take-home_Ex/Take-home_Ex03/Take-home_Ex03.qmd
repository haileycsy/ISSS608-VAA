...
Source column contains vector-like strings with multiple company names, eg. c("Yu er he  Solutions", "Yu er he  Solutions",). This is extracted and split into separate rows: 
...
* Network Graph of Company Contacts also does not reveal any obvious *fishy* patterns
...
visNetwork(
    ...
    links_owner,
    width = "100%",
    main = list(text = "Multiple Companies Seem to be Owned by Several Individuals",
                style = "font-size:17x;
                weight:bold;
                text-align:right;")
  ) %>%
...
* However, a high number of ambiguous services such as "freelance researcher" and "source freelance" were also uncovered. As these may point towards *fishy* activity, these will be extracted for further analysis
* There are several overlaps in the frequent bigrams, that may be grouped together as the same category. The top 20 most frequent bigrams will be used as product/service categories
...
The bigram analysis in section 3.2 was used to deduce categories for products and services. As a company may have listed more than a single product/service, each category is appended as a separate column, with boolean values 1 = TRUE and 0 = FALSE as indication. The categories of interest to the study are:   
  
  
| **Category**         | **Inclusive of: **                                                      |
|----------------------|-------------------------------------------------------------------------|
| **Seafood Products** | fish products, frozen seafood, frozen fish, fresh shellfish, fresh fish |
| **Processing**       | entrails shucking, smoking salting, %processing% , packaging            |
| **Marine Products**  | aquatic products, marine fats, fins scales,                             |
| **Trading**          | freight forwarding, transportation services, logistics services         |
| **Freelance**        | freelance researcher, source freelance                                  |
| **Unknown  **        | Unknown                                                                 |  
...
mc3_nodes_new$Seafood_products <- ifelse(grepl("fish|frozen|shellfish|fresh fish|seafood|sea food", 
                                                mc3_nodes_new$product_services, ignore.case = TRUE), 1, 0)

mc3_nodes_new$Processing <- ifelse(grepl("entrails|shucking|smoking salting|processing|packaging", 
                                                mc3_nodes_new$product_services, ignore.case = TRUE), 1, 0)

mc3_nodes_new$Marine_products <- ifelse(grepl("aquatic products|marine fats|fins|scales", 
                                                mc3_nodes_new$product_services, ignore.case = TRUE), 1, 0)

mc3_nodes_new$Trading <- ifelse(grepl("freight|forwarding|transportation services|logistics services|import|export|trading", 
                                                mc3_nodes_new$product_services, ignore.case = TRUE), 1, 0)

mc3_nodes_new$Freelance <- ifelse(grepl("freelance", mc3_nodes_new$product_services, ignore.case = TRUE), 1, 0)

# Get total count of products/services categories of interest offered by each company
mc3_nodes_new <- mc3_nodes_new %>%
  mutate(cat_count = Seafood_products + Processing + Marine_products + Trading + Freelance)

```

### 3.3.1 Unknown Network
...
```{r}
# Get Companies that do freelance work
nodes_free <- mc3_nodes_new %>%
  filter(Trading == "1")

# Get links to these freelance companies
links_free <- mc3_links_new %>%
  filter(source %in% nodes_free$id) %>%
  rename(from = "source",
         to = "target")
```
...
```{r}
# Get distinct Source and Target
free_source <- links_free %>%
  distinct(from) %>%
  rename("id" = "from")

free_target <- links_free %>%
  distinct(to) %>%
  rename("id" = "to")
```
...
```{r}
# Bind into single dataframe
nodes_free_new <- bind_rows(free_source, free_target)

bo_nodes <- mc3_nodes_new %>%
  filter(type == "Beneficial Owner")

nodes_free_new$group <- ifelse(nodes_free_new$id %in% company_nodes$id, "Company", 
                        ifelse(nodes_free_new$id %in% bo_nodes$id, "Beneficial Owner", "Company Contacts"))
